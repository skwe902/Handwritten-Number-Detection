...
class TrainingImages(QMainWindow):
    transform = transforms.Compose([transforms.ToTensor(),
                              transforms.Normalize((0.5,), (0.5,)),
                              ])

    trainset = datasets.MNIST('mnist_data_train', download=False, train=True, transform=transform)
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=60000, shuffle=True)

    dataiterTrain = iter(trainloader) # creating a iterator
    imagesTrain, labelsTrain = dataiterTrain.next()

    num_of_images = 100
    for index in range(1, num_of_images+1):
        plt.subplot(10, 10, index)
        plt.axis('off')
        plt.imshow(imagesTrain[index].numpy().squeeze(), cmap='gray_r')
    plt.savefig("initTrain.png") #creates an initial image to display 
    ...
    def __init__(self):
        ...
        self.pixmap = QPixmap('initTrain.png')
    ...
    def previousClicked(self):
        ...
        if self.index < 0: #if previous button is pressed when there is no previous image
            ...
            self.index = 0
            pixmap = QtGui.QPixmap("initTrain.png") #show init.png
            ...
            if self.selectedNumber == 10:
                for index in range(1, num_of_images+1):
                    plt.subplot(10, 10, index)
                    plt.axis('off')
                    plt.imshow(TrainingImages.imagesTrain[(index+(100*self.index))].numpy().squeeze(), cmap='gray_r')
                ...
                for index in range(1, num_of_images+1):
                    plt.subplot(10, 10, index)
                    plt.axis('off')
                    if TrainingImages.labelsTrain[(index+(100*self.index))] == self.selectedNumber:
                        plt.imshow(TrainingImages.imagesTrain[(index+(100*self.index))].numpy().squeeze(), cmap='gray_r')
                    else:
                        plt.axis('off')
    ...
    def nextClicked(self):
        ...
        if self.index == 599:
            ...
            if self.selectedNumber ==10: #if the person has selected "show all" option
                ...
                pixmap = QtGui.QPixmap("prev.png") #show prev.png
                self.label.setPixmap(pixmap)
            else: #if the person selects one of the number options
                for index in range(1, num_of_images+1):
                    plt.subplot(10, 10, index)
                    plt.axis('off')
                    if TrainingImages.labelsTrain[(index+(100*self.index))] == self.selectedNumber:
    ...
    def closeEvent(self, event): # when the window is closed
